...
namespace BlueJay.Common.Systems
{
  ...
  public class GamepadSystem : IUpdateSystem
  {
    ...
    /// <param name="provider">The Service provider meant to create objects</param>
    public GamepadSystem(IEventQueue queue, IServiceProvider provider)
    {
      _handlers = new Dictionary<PlayerIndex, GamePadHandler>()
      {
        { PlayerIndex.One, ActivatorUtilities.CreateInstance<GamePadHandler>(provider, queue) },
        { PlayerIndex.Two, ActivatorUtilities.CreateInstance<GamePadHandler>(provider, queue) },
        { PlayerIndex.Three, ActivatorUtilities.CreateInstance<GamePadHandler>(provider, queue) },
        { PlayerIndex.Four, ActivatorUtilities.CreateInstance<GamePadHandler>(provider, queue) }
      };
    }
